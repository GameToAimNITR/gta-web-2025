import type { Member } from './types';

export const animeshTripathy: Member = {
  id: 3,
  name: 'Animesh Tripathy',
  role: 'Code Team',
  year: 2028,
  image: '/members/Animesh_Tripathy.jpg',
  aiHint: 'pixel art code',
  skills: [
    { name: 'Raylib', level: 85 },
    { name: 'Godot', level: 90 },
    { name: 'Game Mechanics', level: 88 },
  ],
  specialAbility: 'Juice Master',
  bio: 'Loves making 2D pixel themed games. Experienced in Raylib and Godot. Really into making good game mechanics and juicing up games with a bit of spaghetti code :)',
  githubUrl: 'https://github.com/Animeshtripathy3',
  linkedinUrl: 'https://www.linkedin.com/in/animesh-tripathy-760799317/',
};
